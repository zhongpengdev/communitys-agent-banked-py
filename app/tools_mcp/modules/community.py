...
async def create_visitor(args: dict) -> dict:
    try:
        data = await http_post("/api/visitor/register", {
        ...
        return error_response(f"访客登记失败: {e}")
...
async def search_goods(args: dict) -> dict:
    try:
        data = await http_post("/api/mall/list", {
        ...
        return error_response(f"商品搜索失败: {e}")
